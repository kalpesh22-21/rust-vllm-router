...
impl Tree {
    /// Returns true when no tenants remain in this tree.
    ///
    /// Workers are stored as tenants; an empty `tenant_char_count` means every
    /// worker has been removed and the `Arc<Tree>` can be dropped from the
    /// outer `DashMap`.
    pub fn is_empty(&self) -> bool {
        self.tenant_char_count.is_empty()
    }
            ...
            if node.children.is_empty() && node.tenant_last_access_time.is_empty() {
                if let Some(ref parent) = parent_opt {
                    ...
                    if let Some(fc) = fc {
                        ...
                        // Break the parent↔child Arc cycle so the detached node
                        // is reclaimed immediately.  No other lock is held here:
                        // the text read-guard above was a temporary (dropped at
                        // end of the statement), and children.remove() has
                        // already released the shard lock.
        ...
        while let Some(curr) = queue.pop_front() {
            ...
            if curr.children.is_empty() && curr.tenant_last_access_time.is_empty() {
                if let Some(ref parent) = parent_opt {
                    ...
                    if let Some(fc) = fc {
                        parent.children.remove(&fc);
                        // Break the parent↔child Arc cycle on the now-detached
                        // node.  The text read-guard is a temporary (dropped at
                        // end of statement) and children.remove() has already
                        // released its shard lock, so no other lock is held.
                        *curr.parent.write().unwrap() = None;
                    }
                }
            }
...
impl TokenTree {
    ...
    pub fn remove_tenant(&self, tenant: &str) {
        ...
        while let Some(curr) = leaves.pop_front() {
            ...
            if curr.children.is_empty() && curr.tenant_last_access_time.is_empty() {
                if let Some(ref parent) = parent_opt {
                    let first = curr.tokens.read().unwrap().first().copied();
                    if let Some(first) = first {
                        parent.children.remove(&first);
                        // Break the parent↔child Arc cycle on the detached node.
                        // The tokens read-guard is a temporary (dropped at end of
                        // statement) and children.remove() has released its shard
                        // lock, so no other lock is held here.
                        *curr.parent.write().unwrap() = None;
                    }
                }
            }
    ...
    pub fn evict_tenant_by_size(&self, max_size: usize) {
        ...
        while let Some(Reverse(entry)) = token_pq.pop() {
            ...
            if node.children.is_empty() && node.tenant_last_access_time.is_empty() {
                if let Some(ref parent) = parent_opt {
                    let first = node.tokens.read().unwrap().first().copied();
                    if let Some(first) = first {
                        parent.children.remove(&first);
                        // Break the parent↔child Arc cycle on the detached node.
                        // The tokens read-guard is a temporary (dropped at end of
