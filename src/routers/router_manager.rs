...
impl RouterManager {
    ...
    pub fn remove_worker_from_registry(
        ...
        // Look up the policy BEFORE calling on_worker_removed: if this is the
        // last worker for the model, on_worker_removed drops the policy entry
        // from the registry, making get_policy return None afterwards.
        let policy = model_id
            .as_deref()
            .and_then(|mid| self.policy_registry.get_policy(mid));
        ...
        if let Some(_worker) = self.worker_registry.remove_by_url(url) {
            ...
            // If the model is using cache-aware policy, prune the worker from
            // the tree.  This must happen after on_worker_removed (so the count
            // is decremented) but the policy Arc was captured above before the
            // entry could be dropped.
            if let Some(policy) = policy {
                    ...
                    .downcast_ref::<crate::policies::CacheAwarePolicy>()
                {
                    ...
                    info!("Removed worker from cache-aware tree: {}", url);
                }
            }
...
mod tests {
    ...
    use crate::core::{BasicWorker, WorkerRegistry, WorkerType};
    use crate::policies::{CacheAwarePolicy, PolicyRegistry};
    use std::collections::HashMap;

    /// Verify that `remove_worker_from_registry` prunes the cache-aware tree.
    ///
    /// Before the fix the policy was looked up AFTER `on_worker_removed`, which
    /// drops the policy entry when the last worker is removed, making
    /// `get_policy` return None and the cache-tree cleanup unreachable.
    #[test]
    fn test_remove_worker_prunes_cache_tree() {
        let model_id = "mgr-model";
        let url = "http://mgr-worker:8080";

        let mut labels = HashMap::new();
        labels.insert("model_id".to_string(), model_id.to_string());

        let worker = Arc::new(
            BasicWorker::new(url.to_string(), WorkerType::Regular).with_labels(labels),
        );
        ...
        let result = manager.remove_worker_from_registry(url);
        assert!(result.is_ok(), "removal must succeed");

        // Worker must be gone from the registry.
        assert!(
            worker_registry.get_by_url(url).is_none(),
            "worker must be removed from registry"
        );

        // The cache tree must have been pruned.
        assert!(
            !cache_policy.has_tree_for_model(model_id),
            "cache tree must be pruned after worker removal via RouterManager"
        );
    }

    /// Removing one of two workers for the same model must leave the other
    /// worker's entry in the tree intact.
    #[test]
    fn test_remove_one_worker_leaves_other_in_cache_tree() {
        let model_id = "mgr-shared-model";
        let url_a = "http://mgr-a:8080";
        let url_b = "http://mgr-b:8080";

        let mut labels = HashMap::new();
        labels.insert("model_id".to_string(), model_id.to_string());

        let wa = Arc::new(
            BasicWorker::new(url_a.to_string(), WorkerType::Regular).with_labels(labels.clone()),
        );
        let wb = Arc::new(
            BasicWorker::new(url_b.to_string(), WorkerType::Regular).with_labels(labels.clone()),
        );

        let worker_registry = Arc::new(WorkerRegistry::new());
        worker_registry.register(wa.clone());
        worker_registry.register(wb.clone());
        ...
        let cache_policy = policy
            ...
            .downcast_ref::<CacheAwarePolicy>()
            .expect("must be CacheAwarePolicy");
        cache_policy.add_worker(wa.as_ref());
        cache_policy.add_worker(wb.as_ref());
        ...
        // Remove only worker A.
        manager.remove_worker_from_registry(url_a).unwrap();

        // The tree for the model must still exist (worker B is still registered).
        assert!(
            cache_policy.has_tree_for_model(model_id),
            "tree must remain while worker B is still registered"
        );
        // Worker A must no longer be in the tree.
        assert!(
            !cache_policy.tree_has_tenant(model_id, url_a),
            "removed worker must be gone from the tree"
        );
        // Worker B must still be in the tree.
        assert!(
            cache_policy.tree_has_tenant(model_id, url_b),
            "remaining worker must still be in the tree"
        );
    }
}
