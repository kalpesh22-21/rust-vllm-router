...
impl GenerationRequest for ChatCompletionRequest {
    ...
    fn extract_text_for_routing(&self) -> String {
                ...
                ChatMessage::User { content, .. } => {
                    match content {
                        ...
                        UserMessageContent::Parts(parts) => {
                            // Extract text from content parts (e.g., text, images)
                            ...
                            if !part_texts.is_empty() {
                                all_texts.push(part_texts.join(" "));
                            }
                        }
                    }
                }
                ChatMessage::Assistant {
                    ...
                    if let Some(reasoning_text) = reasoning {
                        if !reasoning_text.is_empty() {
                            all_texts.push(reasoning_text.clone());
                        }
                    }
                }
                ChatMessage::Tool { content, .. } => {
                    // Include tool results as string representation
        ...
        // Join all texts with spaces to create one long string
