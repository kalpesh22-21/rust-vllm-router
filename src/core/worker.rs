...
pub async fn fetch_kv_utilization(worker_url: &str) -> Result<f32, anyhow::Error> {
    ...
    for line in body.lines() {
        ...
        tracing::debug!(
            "Fetched KV cache utilization for {}: {} (raw: {})",
            worker_url,
            value,
            trimmed
        );
...
pub struct BasicWorker {
    ...
    /// Single-flight guard: true while a background KV-cache refresh is in flight.
    ///
    /// Prevents a thundering herd of concurrent /metrics HTTP calls when the TTL
    /// lapses.  At most one refresh task is spawned per worker at any moment.
    kv_refresh_in_flight: Arc<AtomicBool>,
}
...
impl BasicWorker {
    ...
    pub fn with_connection_mode(
        ...
        Self {
            ...
            kv_refresh_in_flight: Arc::new(AtomicBool::new(false)),
        }
    }
...
impl Worker for BasicWorker {
    ...
    fn kv_cache_utilization(&self) -> f32 {
        ...
        // If the cached value is stale, spawn a single fire-and-forget background
        // task to refresh it.  The CAS on `kv_refresh_in_flight` ensures at most
        // one refresh is in flight per worker at any time, preventing a thundering
        // herd of /metrics HTTP calls when concurrent callers all see a stale value.
        ...
        if last_fetched.elapsed() > KV_TTL {
            if let Ok(handle) = tokio::runtime::Handle::try_current() {
                // Claim the in-flight slot.  Only the winner (false -> true) spawns.
                // Failure ordering is Relaxed: if we lose the CAS we read nothing
                // useful from the flag — we simply skip spawning.
                if self
                    .kv_refresh_in_flight
                    .compare_exchange(false, true, Ordering::AcqRel, Ordering::Relaxed)
                    .is_ok()
                {
                    let url = self.metadata.url.clone();
                    let kv_arc = Arc::clone(&self.kv_cache_utilization);
                    let in_flight = Arc::clone(&self.kv_refresh_in_flight);
                    handle.spawn(async move {
                        // RAII guard: clears the in-flight flag on drop, covering
                        // both normal completion and panics inside this task.
...
mod kv_cache_tests {
    ...
    async fn test_kv_cache_stale_triggers_background_refresh_does_not_block() {
        ...
        let worker = BasicWorker::new("http://127.0.0.1:19999".to_string(), WorkerType::Regular);
    ...
    #[tokio::test]
    async fn test_kv_refresh_in_flight_reset_after_failed_fetch() {
        // The in-flight flag must be cleared after a failed fetch so that
        // subsequent TTL lapses can trigger new refresh attempts.
        let worker = BasicWorker::new("http://127.0.0.1:19999".to_string(), WorkerType::Regular);

        // Flag starts clear.
        assert!(!worker.kv_refresh_in_flight.load(Ordering::Acquire));

        // First call sees a stale timestamp → sets flag and spawns a refresh task.
        let _ = worker.kv_cache_utilization();

        // Note: we do NOT assert the flag is true immediately after the call —
        // on a loaded runner the spawned task may already have completed and
        // cleared it, making that intermediate check inherently racy.

        // Wait for the background task to finish (it will fail the HTTP call to
        // the dummy address and then clear the flag via the RAII InFlightGuard).
        let deadline = std::time::Instant::now() + Duration::from_secs(2);
        while worker.kv_refresh_in_flight.load(Ordering::Acquire) {
            if std::time::Instant::now() > deadline {
                panic!("kv_refresh_in_flight was not reset within 2 s");
            }
            tokio::time::sleep(tokio::time::Duration::from_millis(10)).await;
        }

        // Flag is clear again — a new TTL lapse would be able to trigger another refresh.
        assert!(!worker.kv_refresh_in_flight.load(Ordering::Acquire));
    }

    #[tokio::test]
    async fn test_kv_single_flight_only_one_refresh_spawned() {
        // Calling kv_cache_utilization() many times concurrently on a stale worker
        // must result in at most one in-flight refresh at any given time.
        let worker = Arc::new(BasicWorker::new(
            "http://127.0.0.1:19999".to_string(),
            WorkerType::Regular,
        ));

        // All calls see a stale timestamp.  After all calls complete the flag must
        // be true (one task claimed it) and remain held until the task finishes.
        for _ in 0..20 {
            let _ = worker.kv_cache_utilization();
        }

        // Exactly one task should have claimed the slot.
        assert!(worker.kv_refresh_in_flight.load(Ordering::Acquire));

        // Wait for the single background task to release the flag.
        let deadline = std::time::Instant::now() + Duration::from_secs(2);
        while worker.kv_refresh_in_flight.load(Ordering::Acquire) {
            if std::time::Instant::now() > deadline {
                panic!("kv_refresh_in_flight was not reset within 2 s");
