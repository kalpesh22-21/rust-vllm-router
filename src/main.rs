...
struct CliArgs {
    ...
    /// Token threshold for small request bypass (cache_aware policy)
    #[arg(long, default_value_t = 25000)]
    small_request_token_threshold: usize,

    /// KV utilization threshold for worker filtering (cache_aware policy)
    #[arg(long, default_value_t = 0.9)]
    kv_util_threshold: f32,

    /// Weight for KV utilization in load scoring (cache_aware policy)
    #[arg(long, default_value_t = 0.7)]
    alpha: f32,

    /// Weight for normalized load in load scoring (cache_aware policy)
    #[arg(long, default_value_t = 0.3)]
    beta: f32,
    ...
    fn parse_policy(&self, policy_str: &str) -> PolicyConfig {
        match policy_str {
            ...
            "cache_aware" => PolicyConfig::CacheAware {
                ...
                small_request_token_threshold: self.small_request_token_threshold,
                kv_util_threshold: self.kv_util_threshold,
                alpha: self.alpha,
                beta: self.beta,
            },
