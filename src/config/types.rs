...
fn default_small_request_token_threshold() -> usize {
    25000
}

fn default_kv_util_threshold() -> f32 {
    0.9
}
...
pub enum PolicyConfig {
    ...
    CacheAware {
        ...
        max_tree_size: usize,
        /// Token threshold for small request bypass (default: 25000)
        #[serde(default = "default_small_request_token_threshold")]
        small_request_token_threshold: usize,
        /// KV utilization threshold for worker filtering (default: 0.9)
        #[serde(default = "default_kv_util_threshold")]
        kv_util_threshold: f32,
        /// Weight for KV utilization in load scoring (default: 0.7)
        #[serde(default = "default_alpha")]
        alpha: f32,
        /// Weight for normalized load in load scoring (default: 0.3)
...
mod tests {
    ...
    fn test_cache_aware_parameters() {
        let cache_aware = PolicyConfig::CacheAware {
            ...
            max_tree_size: 5000,
            small_request_token_threshold: 30000,
            kv_util_threshold: 0.85,
            alpha: 0.6,
            beta: 0.4,
        };
        ...
        match cache_aware {
            PolicyConfig::CacheAware {
                ...
                assert_eq!(small_request_token_threshold, 30000);
                assert!((kv_util_threshold - 0.85).abs() < 0.0001);
                assert!((alpha - 0.6).abs() < 0.0001);
                assert!((beta - 0.4).abs() < 0.0001);
            }
